        ...
        >
          <h3>
            <Link
              ...
              class="block underline hover:text-emerald-700 mb-4 text-2xl font-semibold tracking-tight text-slate-200"
            >
              {content?.[0]}
            </Link>
          </h3>
          <h4 class="font-normal text-slate-200 mb-4">{content?.[1]}</h4>
          <p class="text-emerald-500">{content?.[2]}</p>
          {tags && (
            <ul class="flex gap-2 flex-wrap mt-4">
              {tags.split(", ").map((tag) => (
                <li
                  class="text-slate-200 hover:bg-emerald-700 hover:underline mb-2 border-solid border rounded-full text-sm p-2 leading-3 border-gray"
                  key={tag}
                >
