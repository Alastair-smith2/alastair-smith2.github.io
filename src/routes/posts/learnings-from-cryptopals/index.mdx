---
title: Some learnings from cryptopals
author: "Alastair Smith"
description: Some lessons learnt from doing the cryptopal challenge
# Date: 20/03/2023
og:
  - title: Some lessons learnt from doing the cryptopal challenge
    description: true
---
...
From first glance people could be mistaken for thinking somehow this is related to “crypto” in the sense of cryptocurrency. Rather it’s about lessons learnt about cryptography while doing some of the challenges from [https://cryptopals.com/](https://cryptopals.com/) and wider reading in wanting to expand my understanding of cryptography.

## What makes a suitable cryptographic algorithm, a “trapdoor”.<a name="trapdoor"></a>

The majority of cryptographic algorithms in play today that are regarded as relatively secure (e.g. [RSA](<https://en.wikipedia.org/wiki/RSA_(cryptosystem)>), [ECDSA](https://en.wikipedia.org/wiki/Elliptic_Curve_Digital_Signature_Algorithm)) have the element of being like a “trapdoor”. [Trapdoor functions](https://en.wikipedia.org/wiki/Trapdoor_function) are named as such because it’s easy to produce a result in one direction, but difficult to produce the inverse.

## Why cryptographic solutions widely used today are secure, for now.<a name="secure-for-now"></a>

Following on from the above, many of the current “trapdoor functions” leverage integer [factorization](https://en.wikipedia.org/wiki/Integer_factorization), RSA being the prime example. Though in recent times elliptic curves have become a popular alternative where the “trapdoor” relies upon computing discrete logarithms, [https://blog.cloudflare.com/a-relatively-easy-to-understand-primer-on-elliptic-curve-cryptography/](https://blog.cloudflare.com/a-relatively-easy-to-understand-primer-on-elliptic-curve-cryptography/) is an excellent resource on the inner workings and the contrast to factorization. The key point here with these approaches is that current computers do not have sufficient power to produce the inverse in a reasonable time frame. However that’s likely to change in the future as outlined in [https://blog.cloudflare.com/towards-post-quantum-cryptography-in-tls/](https://blog.cloudflare.com/towards-post-quantum-cryptography-in-tls/), with quantum computers in the works, there is a very real need to start creating algorithms that are post-quantum proof. For those interested, [https://blog.cloudflare.com/nist-post-quantum-surprise/](https://blog.cloudflare.com/nist-post-quantum-surprise/) is a great discussion of some of the current challenges around post-quantum cryptography and some of the algorithms that are looking to be standardised.
...
and given some of the attacks that are walked, it’s understandable why the more disconcerting statement is also made:
...
Some of the ways aren’t obvious until working through the challenges, [even little clues like a server response taking slightly longer based on encryption/ decryption flow have been used to decrypt ciphertexts](https://en.wikipedia.org/wiki/Padding_oracle_attack), although thankfully that attack method isn’t as widely susceptible nowadays.

## Implementing Diffie-Hellman<a name="diffie-hellman"></a>

This is the activity I enjoyed most, albeit one of the more simpler challenges. The problem around factorization really sunk in while learning about modpow in [Rust](https://docs.rs/num-bigint/latest/num_bigint/struct.BigUint.html#method.modpow).
