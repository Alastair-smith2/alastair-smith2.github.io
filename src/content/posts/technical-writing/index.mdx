...
description: Learnings from Google's technical writing course
date: January 2025
tags:
  - Writing
og:
...
4. [Ambiguous pronouns](#ambiguous-pronouns)
...
## Ambiguous pronouns<a name="ambiguous-pronouns"></a>

Pronouns are nouns that are intended to replace or point to another noun. Nouns are abundantly present throughout writing and used to identify people, places, or things. An example from the course would be "Sam runs races. He likes to compete". Sam is the noun here and "he" is the pronoun. But in the following example "We went to the beach on a hot sunny day. It was delightful" what the "it" is referring to isn't clear. Was "it" meant to be about the beach, the hot sunny day or a combination of the two? Which noun is being replaced by "it" in this context isn't clear. Another example would be "Team Bulbasaur are replacing team Charmander as the owners for the catalog service. It has been a real pleasure working with them". Does "them" refer to team Charmander or team Bulbasaur? "It", "they", "them" and "their" are apparently the most common source of confusion, which aligns with my experience. In my mind most cases of ambiguous pronouns result from people trying to be concise but accidentally causing more work for the reader.

## Active voice<a name="active-voice"></a>

The course recommends that most technical documentation should be written in the active voice. The following example shows the typical difference between active and passive voices.
...
The above is taken directly from Google's course. An example of a subordinate clause would be "considered an awful language by many" in the following sentence "JavaScript, considered an awful language by many, was created in 10 days". A subordinate clause doesn't make sense by itself without the main clause. I found it surprising that this advice is marked as optional in the course. The reason is that a lot of subordinate clauses I've read in documentation are irrelevant. In my mind authors often add multiple subordinate clauses as an "oh yeah these bits of extra information will be helpful" but often they cloud the important detail.
